...
def sysArgument():
    ...
    ap.add_argument("--url", help="target URL Path")

    args = vars(ap.parse_args())

    # Automation for this machine
    if args['username'] is None:
        args['username'] = 'michelle'
    if args['password'] is None:
        args['password'] = 'michelle'
    if args['url'] is None:
        args['url'] = 'http://dms-pit.htb/seeddms51x/seeddms'
...
def getDocID(sessionObj, url, docName):
    ...
    path = "/out/out.ViewFolder.php?action=folderList&folderid=8&orderby=u"
    url += path
